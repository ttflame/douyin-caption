...
import { ArrowLeft, BookmarkPlus, RefreshCw, Save, Trash2 } from 'lucide-vue-next'
...
function selectLength(value: number) { form.settings.targetLengthMode = 'fixed'; form.settings.targetCharacters = value }
...
async function savePreset() { if (!presetName.value.trim()) return; presetSaving.value = true; error.value = ''; try { const preset = await api.createPreset(presetName.value.trim(), { ...form.settings }); presets.value = [preset, ...presets.value]; selectedPresetId.value = preset.id; presetName.value = preset.name } catch (reason) { error.value = reason instanceof Error ? reason.message : '预设保存失败' } finally { presetSaving.value = false } }
async function deletePreset() { if (!selectedPresetId.value) return; try { await api.deletePreset(selectedPresetId.value); presets.value = presets.value.filter((item) => item.id !== selectedPresetId.value); selectedPresetId.value = ''; presetName.value = '' } catch (reason) { error.value = reason instanceof Error ? reason.message : '预设删除失败' } }
async function persistSelectedPreset() { if (!selectedPresetId.value || !presetName.value.trim()) return; const updated = await api.updatePreset(selectedPresetId.value, presetName.value.trim(), { ...form.settings }); presets.value = presets.value.map((item) => item.id === updated.id ? updated : item); presetName.value = updated.name }
async function updatePreset() { if (!selectedPresetId.value || !presetName.value.trim()) return; presetSaving.value = true; error.value = ''; try { await persistSelectedPreset() } catch (reason) { error.value = reason instanceof Error ? reason.message : '预设更新失败' } finally { presetSaving.value = false } }
...
<template>
  <div class="page narrow-page">
    ...
    <form v-else class="creation-form" @submit.prevent="submit">
      <section class="preset-toolbar"><label><span class="preset-label">个人创作预设<small v-if="presetDirty">有未保存修改</small></span><select v-model="selectedPresetId" class="field-control" @change="applyPreset"><option value="">不使用预设</option><option v-for="preset in presets" :key="preset.id" :value="preset.id">{{ preset.name }}</option></select></label><label><span>预设名称</span><input v-model="presetName" class="field-control" placeholder="输入预设名称" /></label><UiButton v-if="selectedPresetId" :icon="RefreshCw" :loading="presetSaving" :disabled="!presetName.trim() || !presetDirty" @click="updatePreset">更新预设</UiButton><UiButton v-else :icon="BookmarkPlus" :loading="presetSaving" :disabled="!presetName.trim()" @click="savePreset">保存预设</UiButton><button class="icon-button" type="button" title="删除所选预设" aria-label="删除所选预设" :disabled="!selectedPresetId" @click="deletePreset"><Trash2 :size="17" /></button></section>
      ...
      <section class="form-section"><div class="section-heading"><h2>输出规格</h2><span>{{ form.settings.targetLengthMode === 'follow_source' ? `跟随原文（当前 ${form.sourceText.length} 字）` : '按中文字符控制' }}</span></div><UiField v-model="form.settings.targetCharacters" label="目标字数" type="number" :min="1" :max="20000" :disabled="form.settings.targetLengthMode === 'follow_source'" :hint="form.settings.targetLengthMode === 'follow_source' ? '生成稿将保持与原文大致相当的篇幅' : ''" @update:model-value="form.settings.targetLengthMode = 'fixed'" /><div class="choice-row" aria-label="长度方式"><button class="choice-button" :class="{ active: form.settings.targetLengthMode === 'follow_source' }" type="button" @click="form.settings.targetLengthMode = 'follow_source'">跟随原文</button><button v-for="value in lengths" :key="value" class="choice-button" :class="{ active: form.settings.targetLengthMode === 'fixed' && form.settings.targetCharacters === value }" type="button" @click="selectLength(value)">{{ value }} 字</button></div></section>
      <section class="form-section"><div class="section-heading"><h2>人物与受众</h2><span>决定说话位置</span></div><div class="two-columns"><UiField v-model="form.settings.persona" label="人设背景" multiline :rows="4" /><UiField v-model="form.settings.audience" label="目标受众" multiline :rows="4" /></div></section>
      <section class="form-section"><div class="section-heading"><h2>表达方式</h2><span>决定成品听感</span></div><div class="two-columns"><UiField v-model="form.settings.languageStyle" label="语言风格" multiline :rows="4" /><UiField v-model="form.settings.contentStructure" label="内容结构" multiline :rows="4" /></div></section>
      <section class="form-section"><div class="section-heading"><h2>输出与约束</h2><span>明确不能偏离的边界</span></div><div class="two-columns"><UiField v-model="form.settings.outputSpecification" label="输出要求" multiline :rows="4" /><UiField v-model="form.settings.hardConstraints" label="强制约束" multiline :rows="4" /></div></section>
